...
public:
   ...
   BOOL IsUserFramesOnBoard() { return m_IsUserFramesOnBoard; }
...
protected:
   ...
   BOOL m_IsUserFramesOnBoard;   // Indicates if the number of frames on board was specified by application code
};
...
public:
   ...
   } MULTI_THREADED_CALLBACK_INFO, *PMULTI_THREADED_CALLBACK_INFO;
...
public:
   ...
   void SetCountThrashFrame(BOOL bIgnoreThrashFrame);
   BOOL GetCountThrashFrame() { return m_bCountThrashFrame; }
...
protected:
   ...
   // Multi-threaded transfer callback management
   static SAPSTATUS CCONV OnMultiThreadedNormalEx(void *context, COREVENTINFO eventInfo);
   static SAPSTATUS CCONV OnMultiThreadedNormal(void *context, UINT32 eventType, UINT32 eventCount);
   static SAPSTATUS CCONV OnMultiThreadedTrash(void *context, UINT32 eventType, UINT32 eventCount);
   static UINT WINAPI CallbackThreadProc(LPVOID lpParam);
...
protected:
   ...
   BOOL m_bCountThrashFrame; //TRUE if thrashed frame are considered for frame rate statistic

   // Multi-threaded transfer callback management
   BOOL m_IsMultiThreadedCallback;
   std::queue<MULTI_THREADED_CALLBACK_INFO> m_MultiThreadedCallbackInfo;   // Local callback queue
   HANDLE m_MultiThreadedCallbackSync; // Synchronization of access to local callback queue
   HANDLE m_hCallbackThread;  // Callback thread handle
   HANDLE m_hStartEvent;      // Signaled by the callback thread after it is started
   HANDLE m_hExitEvent;       // When signaled, causes the callback thread to exit
   HANDLE m_hCallbackEvent;   // Signaled when a callback is ready to be processed
};
