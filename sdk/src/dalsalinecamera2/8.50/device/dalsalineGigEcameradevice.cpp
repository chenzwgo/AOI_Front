#include "dalsalineGigecameradevice.h"
#include "rtdefine.h"
#include "trace.h"
...
dalsalineGigecameradevice::dalsalineGigecameradevice()
    : m_AcqDevice(NULL)
    , m_pFeature(NULL)
...
bool dalsalineGigecameradevice::init(const char *szDeviceName)
{
    m_strDeviceName = szDeviceName;
    return true;
}
bool dalsalineGigecameradevice::openDevice()
{
	bool bRet = false;
	do
	{
...
bool dalsalineGigecameradevice::openDevice(SapAcqDevice    *pAcqDevice)
{
    bool bRet = false;
    do 
    {
...
bool dalsalineGigecameradevice::closeDevice()
{
    bool bRet = false;
    do 
    {
...
bool dalsalineGigecameradevice::IsOpen()
{
    RETURN_V_IF(m_bIsUnknown, true); //相机未知不操作相机直接返回正常
    return true;
}

bool dalsalineGigecameradevice::setTDIFeature(bool open)
{
	if (m_AcqDevice == NULL)
	{
		return false;
...
bool dalsalineGigecameradevice::SetGain(double GainValue)
{
    bool bRet = false;
	if (m_AcqDevice == NULL)
	{
...
bool dalsalineGigecameradevice::SetExposureTime(double ExpTimeValue)
{
    bool bRet = false;
	if (m_AcqDevice == NULL)
	{
...
bool dalsalineGigecameradevice::SetCameraFrequency(unsigned int Freq)
{
    bool bRet = false;
    if (Freq < 1)
    {
...
bool dalsalineGigecameradevice::SetTriggerMode(bool triger)
{
    bool bRet = false;
	if (m_AcqDevice == NULL)
	{
...
bool dalsalineGigecameradevice::setCamPluginId(string strCamPluginID)
{
