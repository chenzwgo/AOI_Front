...
#define CORMAN_SETTING_CBK_FIFO_SIZE_DEFAULT				4096
...
#define CORMAN_SETTING_ENABLE_PARALLEL_XFER_CBK         7
#define CORMAN_SETTING_ENABLE_PARALLEL_XFER_CBK_STR     "Enable Parallel Transfer Callbacks"
#define CORMAN_SETTING_ENABLE_PARALLEL_XFER_CBK_DEFAULT 0
...
/*-----------------------------------------------------------------------------
 * CorManGetServerByDeviceUserId
 *
 * Description:
 *		Return an handle using its device user ID in the server database
 *
 *	Parameters:
 *    deviceUserId User ID for server
 *		name	       Returned location name
 *		handle       Returned handle
 *-----------------------------------------------------------------------------
 */

CORSTATUS CORAPIFUNC CorManGetServerByDeviceUserId( PCSTR deviceUserId, PSTR name, PCORSERVER server);
typedef CORSTATUS (CCONV *CORMANGETSERVERBYDEVICEUSERID) (PCSTR deviceUserId, PSTR name, PCORSERVER server);
 ...
 *    serverName: 	server's name
 *  	location:		SAPERA address
 *    deviceUserId:  user defined ID of the server
 *-----------------------------------------------------------------------------
 */
typedef struct
{
   UINT32 version;         // 1 = deviceUserId only
   const char* deviceUserId;
} CORSERVER_ADDITIONAL_INFO, *PCORSERVER_ADDITIONAL_INFO;

CORSTATUS CORAPIFUNC CorManAddServerToDatabaseEx( const char serverName[], UINT32 location, PCORSERVER_ADDITIONAL_INFO pServerAdditionalInfo);
typedef CORSTATUS (CCONV *CORMANADDSERVERTODATABASEEX) (const char serverName[], UINT32 location, PCORSERVER_ADDITIONAL_INFO pServerAdditionalInfo);
