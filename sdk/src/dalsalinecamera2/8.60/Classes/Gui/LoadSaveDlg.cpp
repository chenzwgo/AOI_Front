...
TCHAR* CLoadSaveDlg::m_TiffCompressionTypes[] =
{
   _T("corfile_val_compression_none"),
   _T("corfile_val_compression_rle"),
   _T("corfile_val_compression_lzw"),
   _T("corfile_val_compression_jpeg"),
};
...
{//{FORMAT,               {BMP,           TIFF,          CRC,           RAW,           JPEG,          JPEG2000     }}
	...
	{SapFormatRGBAP8,      {UNSUPPORTED,   UNSUPPORTED,   NO_CONVERSION, NO_CONVERSION, UNSUPPORTED,   UNSUPPORTED}},
	{SapFormatRGBP16,      {CONV_TO_RGB10, CONV_TO_RGB16, NO_CONVERSION, NO_CONVERSION, UNSUPPORTED,   UNSUPPORTED}},
	{SapFormatRGBAP16,     {UNSUPPORTED,   UNSUPPORTED,   NO_CONVERSION, NO_CONVERSION, UNSUPPORTED,   UNSUPPORTED}},
	{SapFormatBICOLOR88,   {CONV_TO_RGB8,  CONV_TO_RGB8,  NO_CONVERSION, NO_CONVERSION, UNSUPPORTED,   UNSUPPORTED}},
	{SapFormatBICOLOR1616, {CONV_TO_RGB8,  CONV_TO_RGB8,  NO_CONVERSION, NO_CONVERSION, UNSUPPORTED,   UNSUPPORTED}},
	{SapFormatBICOLOR1212, {CONV_TO_RGB8,  CONV_TO_RGB8,  NO_CONVERSION, NO_CONVERSION, UNSUPPORTED,   UNSUPPORTED}},
	{SapFormatRGB888_MONO8, {UNSUPPORTED,   UNSUPPORTED,   NO_CONVERSION, NO_CONVERSION, UNSUPPORTED,   UNSUPPORTED}},
	{SapFormatRGB161616_MONO16, {UNSUPPORTED,   UNSUPPORTED,   NO_CONVERSION, NO_CONVERSION, UNSUPPORTED,   UNSUPPORTED}},
	{SapFormatMono8P2,     {UNSUPPORTED,   UNSUPPORTED,   NO_CONVERSION, NO_CONVERSION, UNSUPPORTED,   UNSUPPORTED}},
	{SapFormatMono8P3,     {UNSUPPORTED,   UNSUPPORTED,   NO_CONVERSION, NO_CONVERSION, UNSUPPORTED,   UNSUPPORTED}},
	{SapFormatMono8P4,     {UNSUPPORTED,   UNSUPPORTED,   NO_CONVERSION, NO_CONVERSION, UNSUPPORTED,   UNSUPPORTED}},
	{SapFormatMono16P2,    {UNSUPPORTED,   UNSUPPORTED,   NO_CONVERSION, NO_CONVERSION, UNSUPPORTED,   UNSUPPORTED}},
	{SapFormatMono16P3,    {UNSUPPORTED,   UNSUPPORTED,   NO_CONVERSION, NO_CONVERSION, UNSUPPORTED,   UNSUPPORTED}},
	{SapFormatMono16P4,    {UNSUPPORTED,   UNSUPPORTED,   NO_CONVERSION, NO_CONVERSION, UNSUPPORTED,   UNSUPPORTED}}
};
...
: CFileDialog(isFileOpen, _T(""), NULL, OFN_HIDEREADONLY | (isFileOpen ? OFN_FILEMUSTEXIST : OFN_OVERWRITEPROMPT), isSequence ? _T(SEQUENCE_FILTER) : _T(STANDARD_FILTER), pParent)
{
	m_pBuffer = pBuffer;
	m_App = AfxGetApp();
...
BOOL CLoadSaveDlg::UpdateBuffer()
{
   ...
   if (!SetOptions())
	...
	if (m_pBuffer != NULL)
	{
		...
		{
			return m_pBuffer->Load(CStringA(m_PathName), m_StartFrame, 0, 0, CStringA(m_Options));
		}
		else
		{
            ...
            MessageBox(message, 0, MB_ICONEXCLAMATION);
			}
			return m_pBuffer->Save(CStringA(m_PathName), CStringA(m_Options), m_StartFrame);
		}
	}
...
{
	// Read file name
	CString str = m_App->GetProfileString(_T("SapFile"), _T("Name"), _T(""));
   _tCorStrncpy(m_PathName, str, sizeof(m_PathName)); 

	// Read file type
	m_Type = m_App->GetProfileInt(_T("SapFile"), _T("Type"), 0);
}

void CLoadSaveDlg::SaveSettings()
{
	// Write file name
	m_App->WriteProfileString(_T("SapFile"), _T("Name"), CString(m_PathName));

	// Write file type
	m_App->WriteProfileInt(_T("SapFile"), _T("Type"), m_Type);
}
...
BOOL CLoadSaveDlg::AddTiffOptions()
{
   ...
   if (dlg.DoModal() != IDOK)
	...
	CString strType;
   strType.Format(_T(" -compression %s"), m_TiffCompressionTypes[dlg.GetCompressionType()]);
	m_Options += strType;

	CString strLevel;
	strLevel.Format(_T(" -quality %d"), dlg.GetCompressionLevel());
	m_Options += strLevel;
...
BOOL CLoadSaveDlg::AddRawOptions()
{
   ...
   m_RawFileFormat = dlg.GetRawFileFormat();
	...
	CString str;
	str.Format(_T(" -width %d -height %d -offset %d"), m_RawFileWidth, m_RawFileHeight, m_RawFileOffset);
	m_Options += str;
...
BOOL CLoadSaveDlg::AddJpegOptions()
{
   ...
   if (dlg.DoModal() != IDOK)
	...
	CString str;
	str.Format(_T(" -q %d"), dlg.GetJpegFileQuality());
	m_Options += str;
