...
CDalsaLineCamera::CDalsaLineCamera(const char *pszGUID)
    : m_pInParam(NULL)
...
//    , m_nPixelColorFilter(GX_COLOR_FILTER_NONE)
{
    ...
    m_pThreadOpenCam = new JThreadTP<CDalsaLineCamera,void*,void>;
}


CDalsaLineCamera::~CDalsaLineCamera(void)
{
    m_bIsRunning = false;
...
bool CDalsaLineCamera::InitCam(unsigned int ServerIndex, unsigned int ResourceIndex, const char *szDeviceName, const LINE_CAM_PARAM *param)
{
...
//}
const unsigned char *CDalsaLineCamera::GetImage()
{
    return NULL;
...
bool CDalsaLineCamera::SingleAcq(bool bSoft)
{
    if ((!IsOpen() && !Open()))
    {
        return false;
    ...
    try
    {
        ...
        if (NULL != m_Xfer)
        {
            ...
            {
                bRet = m_Acq->SoftwareTrigger(SapAcquisition::SoftwareTriggerExt);
            }
            else
            {
                if (!m_Xfer->IsGrabbing())
...
bool CDalsaLineCamera::StartAcq()
{
    if ((!IsOpen() && !Open()))
    {
        return false;
    ...
    try
    {
        if (NULL != m_Xfer)
        {
            ...
            if (m_CameraParam.blockSize != height)
            {
                ...
                m_Xfer->Destroy();
                m_Acq->SetParameter(CORACQ_PRM_CROP_HEIGHT, m_CameraParam.blockSize); //
                m_Xfer->Create();
            }
            if (!m_Xfer->IsGrabbing())
            {
...
bool CDalsaLineCamera::GetImageSize(int &w, int &h)
{
    if (!IsOpen() && !Open())
    {
        return false;
...
bool CDalsaLineCamera::AbortAcq()
{
...
bool CDalsaLineCamera::Open()
{
    ...
    try
    {
        do 
        {
            ...
            m_Buffer        = new SapBufferWithTrash(1, m_Acq);
            m_Xfer          = new SapAcqToBuf(m_Acq, m_Buffer, CDalsaLineCamera::BufferCallback, this); //TBM
			...
			if (NULL != m_camDevice && !m_camDevice->openDevice(loc, m_CameraParam.strCamConfigPath.c_str()))
			{
				break;
			}
            if (NULL == m_Acq || !m_Acq->Create())
                break;
            ...
            {
				//---
               // m_camDevice->setTDIFeature(m_CameraParam.bIsTDI);  4K没有TDI 技术屏蔽
            }
            //int value = 0;
...
bool CDalsaLineCamera::IsOpen()
{
    CISAPUGuard<JMutex> locker(m_lock);
    if (NULL == m_Acq || NULL == *m_Acq)
    {
        return false;
    }
...
bool CDalsaLineCamera::SetGain( double GainValue )
{
    if (GainValue < 1.0)
    {
        return false;
...
bool CDalsaLineCamera::SetExposureTime( double ExpTimeValue )
{
    bool bRet = false;
    if (ExpTimeValue < 1.0)
    {
...
bool CDalsaLineCamera::SetCameraFrequency(unsigned int Freq)
{
    bool bRet = false;
    if (Freq < 1)
    {
...
bool CDalsaLineCamera::CreateImageBuffer()
{
    unsigned long width = 0;
    unsigned long height = 0;
    m_Acq->GetParameter(CORACQ_PRM_CROP_WIDTH, &width);
    m_Acq->GetParameter(CORACQ_PRM_CROP_HEIGHT, &height);
...
bool CDalsaLineCamera::InsertBuffer( void *pData, int width, int height/* = 1*/)
{
	...
	m_iCurrentIndex++;
	m_iRecvCount += height;
	LOG_INFO_F_SIMP("Trace_DalsaLineCamera.log","-------> InsertBuffer(%s-total:%d)---%d--%d)", m_CameraParam.strCameraID.c_str(), m_iRecvCount, m_iCurrentIndex, height);
...
void CDalsaLineCamera::ThreadInitCam(void* param)
{
    if (!IsOpen())
    {
        Open();
...
/**相机掉线处理
*********************************************************************/
void CDalsaLineCamera::ProcessOffline()
{
}

void __stdcall CDalsaLineCamera::OnDeviceOfflineCallbackFun( void* pUserParam )
{
    CDalsaLineCamera *pCtrDevice = (CDalsaLineCamera *)pUserParam;
    if (NULL != pCtrDevice)
    {
...
bool CDalsaLineCamera::IsImplemented()
{
    m_bIsColorFilter = false;
    //if (NULL != m_Buffers)
    {
...
void CDalsaLineCamera::TransformToRGB( void *pBuffer, void *pR, void *pG, void *pB )
{
}

bool CDalsaLineCamera::SetCameraInParam( LINE_CAM_PARAM &param )
{
    bool bRet = false;
    do 
    {
        ...
        {
            int blockSize = 0;
            m_Acq->GetParameter(CORACQ_PRM_CROP_HEIGHT, &blockSize);
            if (blockSize != param.blockSize && NULL != m_Xfer && m_Xfer->IsGrabbing())
            {
                m_Acq->SetParameter(CORACQ_PRM_CROP_HEIGHT, param.blockSize);
            }
        }
        ...
        if (m_CameraParam.height != param.height && NULL != m_Acq && NULL != m_Xfer)
        {
            if (*m_Xfer)
            {
                m_Xfer->Destroy();
        ...
        if (IsOpen() || Open())
        {
            ...
            {
                m_Acq->SetParameter(CORACQ_PRM_EXT_TRIGGER_DETECTION, param.tgDetection);
...
void CDalsaLineCamera::ClearBuf()
{
    m_iCurrentIndex = 0;
	m_iRecvCount = 0;
}

bool CDalsaLineCamera::Continuations(bool bOpen, int iSize/* = 1*/)
{
    bool bRet = true;
    if ((!IsOpen()))
    {
    ...
    if (NULL != m_Xfer)
    {
        ...
        if (bOpen)
        {
            ...
            if (height != iSize)
            {
                ...
                m_Xfer->Destroy();
                m_Acq->SetParameter(CORACQ_PRM_CROP_HEIGHT, iSize); //
                m_Xfer->Create();
                CreateImageBuffer();
            }
            m_bIsGrab = true;
...
bool CDalsaLineCamera::IsContinuations()
{
    bool bRet = true;
    if ((!IsOpen() && !Open()))
    {
...
void CDalsaLineCamera::BufferCallback( SapXferCallbackInfo * pInfo)
{
    CDalsaLineCamera *pInstance = (CDalsaLineCamera *)pInfo->GetContext();
    ...
    pInstance->InsertBuffer(bData, iWidth, height);
...
int CDalsaLineCamera::GetCamStatus() 
{
    CISAPUGuard<JMutex> locker(m_camStatusLock); 
    return m_CamStatus;
};
void CDalsaLineCamera::setCamStatus(unsigned long status)
{
    CISAPUGuard<JMutex> locker(m_camStatusLock);
...
int CDalsaLineCamera::GetLastError(char *errInfo/* = NULL*/, int *iSize/* = NULL*/)
{
    //int iLength = 0;
    if (NULL != iSize)
    {
...
void CDalsaLineCamera::SetTriggerFunction( FnLineTriggerFunction fnTriggerFunction, void *pInParam )
{
    CISAPUGuard<JMutex> locker(m_lock);
...
bool CDalsaLineCamera::IsInitFinished()
{
    return !m_pThreadOpenCam->IsRunning();
}

bool CDalsaLineCamera::IsSnapFinished()
{
    bool bRet = true;
    if ((!IsOpen() && !Open()))
    {
...
bool CDalsaLineCamera::WaitGetImageFinished(int iTimeOut)
{
    long time = iTimeOut; 
    //if (iTimeOut < 0)
    //{
...
bool CDalsaLineCamera::ResetFinishState()
{
    CISAPUGuard<JMutex> locker(m_lock);
    m_ReadImageEvent.ResetEvent();
    m_GrabFinishedEvent.ResetEvent();
...
bool CDalsaLineCamera::SetTriggerMode( bool triger )
{
	bool bRet = false;
	do 
    {
...
void CDalsaLineCamera::ClearRecvBuffer()
{
}
