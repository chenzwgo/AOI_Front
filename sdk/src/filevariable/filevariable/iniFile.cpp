...
void CIniFile::Initialize(string const iniPath)
{
	Path(iniPath);
	caseInsensitive = true;
}
...
bool CIniFile::ReadFile()
{
  ...
  while( getline( f, line)) {
	...
	//new add by ycy 2015-5-4
	//cout << "line  = " << line << "\t" << "line_len = " << line.length() << endl;
...
bool CIniFile::SetValueI( string const keyname, string const valuename, int const value, bool const create)
{
  ...
  snprintf( svalue, MAX_VALUEDATA, "%d", value);
  return SetValue( keyname, valuename, svalue);
}

bool CIniFile::SetValueF( string const keyname, string const valuename, float const value, bool const create)
{
  char svalue[MAX_VALUEDATA];

  snprintf( svalue, MAX_VALUEDATA, "%f", value);
  return SetValue( keyname, valuename, svalue);
}


bool CIniFile::SetValueD( string const keyname, string const valuename, double const value, bool const create /*= true*/ )
{
	char svalue[MAX_VALUEDATA];
...
bool CIniFile::SetValueV( string const keyname, string const valuename, char *format, ...)
{
  ...
  snprintf( value, MAX_VALUEDATA, format, args);
  va_end( args);
...
int CIniFile::GetValueI(string const keyname, string const valuename, int const defValue) const
{
  ...
  snprintf( svalue, MAX_VALUEDATA, "%d", defValue);
  return atoi( GetValue( keyname, valuename, svalue).c_str()); 
}

unsigned long CIniFile::GetValueul( string const keyname, string const valuename, unsigned long const defValue /*= 0*/ ) const
{
	char svalue[MAX_VALUEDATA] = {0};
    snprintf( svalue, MAX_VALUEDATA, "%ld", defValue);
	string strtst = GetValue( keyname, valuename, svalue);
	unsigned long ultest = strtoul(strtst.c_str(),NULL,0);
	return ultest;
}


float CIniFile::GetValueF(string const keyname, string const valuename, float const defValue) const
{
  char svalue[MAX_VALUEDATA];

  snprintf( svalue, MAX_VALUEDATA, "%f", defValue);
  return atof( GetValue( keyname, valuename, svalue).c_str()); 
}


double CIniFile::GetValueD( string const keyname, string const valuename, double const defValue /*= 0.0*/ ) const
{
	char svalue[MAX_VALUEDATA];
