...
bool CMyCamera::IsOpen()
{
    ...
    int iErr = MV_CC_GetDeviceInfo(m_hDevHandle, &stDeviceInfo);
    if (MV_OK != iErr)
    {
    return false;
    }*/
	/*MVCC_ENUMVALUE nValue;
	int iErr = MV_CC_GetEnumValue(m_hDevHandle, "DeviceConnectionStatus", &nValue);
	if (MV_OK != iErr)
    {
        return false;
    }
	if(nValue.nCurValue == 0 && m_bStartGrabbing)
	{
		return true;
	}*/
    //读取任意一个值判断相机是否在线
    /*MVCC_INTVALUE nValue;
    int nRet = MV_CC_GetIntValue(m_hDevHandle, "Width", &nValue);
    if (MV_OK != nRet)
    {
        return false;
    }*/
...
int CMyCamera::StartGrabbing()
{
	...
	int iRet = MV_CC_StartGrabbing(m_hDevHandle);
...
int CMyCamera::ConvertPixelType( MV_CC_PIXEL_CONVERT_PARAM* pstParam )
{
    if (NULL == pstParam)
    {
        return MY_FAIL;
    ...
    return MV_CC_ConvertPixelType(m_hDevHandle, pstParam);
}

// 注册图像数据回调
int CMyCamera::RegisterImageCallBack(void(__stdcall* cbOutput)(unsigned char * pData, MV_FRAME_OUT_INFO_EX* pFrameInfo, void* pUser), void* pUser)
{
    if (NULL == m_hDevHandle)
    {
        return MY_FAIL;
...
// 获取Int型参数，如 Width和Height，详细内容参考SDK安装目录下的 HikCameraNode.xlsx 文件
...
// 设置Int型参数，如 Width和Height，详细内容参考SDK安装目录下的 HikCameraNode.xlsx 文件
...
// 获取Float型参数，如 ExposureTime和Gain，详细内容参考SDK安装目录下的 HikCameraNode.xlsx 文件
...
// 设置Float型参数，如 ExposureTime和Gain，详细内容参考SDK安装目录下的 HikCameraNode.xlsx 文件
...
// 获取Enum型参数，如 PixelFormat，详细内容参考SDK安装目录下的 HikCameraNode.xlsx 文件
...
// 设置Enum型参数，如 PixelFormat，详细内容参考SDK安装目录下的 HikCameraNode.xlsx 文件
...
// 获取Bool型参数，如 ReverseX，详细内容参考SDK安装目录下的 HikCameraNode.xlsx 文件
...
// 设置Bool型参数，如 ReverseX，详细内容参考SDK安装目录下的 HikCameraNode.xlsx 文件
...
// 获取String型参数，如 DeviceUserID，详细内容参考SDK安装目录下的 HikCameraNode.xlsx 文件UserSetSave
...
// 设置String型参数，如 DeviceUserID，详细内容参考SDK安装目录下的 HikCameraNode.xlsx 文件UserSetSave
