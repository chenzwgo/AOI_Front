...
int  CSocketClient::createSocket()
{
   ...
   // m_pstSokcet->iSocket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    if (m_Socket.iSocket == INVALID_SOCKET)
    {
		...
		LOG_WARN_F(SOCKET_CLIENT_LOG, "create socket return -1 ,error code = %d", iError);
...
//    ::bind(m_Socket.iSocket,(struct sockaddr*)(&SockAdd), sizeof(SockAdd));
    ...
    const int yes = 1;
    setsockopt( m_Socket.iSocket, SOL_SOCKET, SO_REUSEADDR, (const char*)&yes, sizeof(yes));
...
int CSocketClient::connectSocket()
{
    ...
    {
		int iError = WSAGetLastError();
		LOG_WARN_F(SOCKET_CLIENT_LOG, "bind return -1 ,ip = %s,port = %d,error code = %d", m_strClinetIPAddr.c_str(), m_iClinetPort, iError);
        CSocketCommon::closeSocket(&m_Socket);
...
#ifdef Q_OS_WIN
               int iError = WSAGetLastError();
               if (WSAENETDOWN == iError || WSAESHUTDOWN == iError)
               {
                   CSocketCommon::closeSocket(pstSocket);
#endif
