...
{
    CSocketproxy* pSocketProxy = dynamic_cast<CSocketproxy*>(m_pClientListCfg->getSocketClientProxy(m_strCameraName.toStdString()));
    if (NULL == pSocketProxy)
    {
        PrintLog(QString::fromLocal8Bit("pSocketProxy"), UILOG_ERR);
        return;
    }
    CSocketClient* pISMCameraClient = pSocketProxy->getSocketClient();
...
void CommunicationTestForm::SendContext( const STSendContext& stContext )
{
    ...
    CSocketClient* pSocketClient = pSocketProxy->getSocketClient();
