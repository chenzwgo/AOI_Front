...
int CSocketproxy::processRecvCtrlPacket(stPacket* packet)
{
	...
	if (packet->fun == PACKET_FUN_SEND_PRODUCT_DATA_RESULT)
	{
		//"{\"productSn\": \"%d\",\"data\": \"%s\"}"
		unsigned int iproductSn = 0;
		...
		if (mapBody.find("data") != mapBody.end())
		{
			strdata=(mapBody["data"]);
		}
		if (NULL != m_pReportCenter)
		{
			m_pReportCenter->reportSendTo("mainui", PACKET_FUN_SEND_PRODUCT_DATA_RESULT, (OS_LPARAM)iproductSn, (OS_LPARAM)&strdata);
		}
	}
    else if (packet->fun == PACKET_FUN_SEND_PRODUCT_SN)
    {
        //"{\"productSn\": \"%d\"}"
        unsigned int iproductSn =0;
        unsigned int ires = 0;
	...
	else if (packet->fun == PACKET_FUN_SYS_PAUSE)
	{
		if (NULL != m_pReportCenter)
		{
			m_pReportCenter->reportSendTo("mainui", "pause");
		}
		packet->body = "";
	}
    ...
    else if (packet->fun == PACKET_FUN_QUERY_SYS_STATUS)
    {
		packet->body = "";
		packet->status = PACKET_STATUS_OK;
		int status = 0;
		if (NULL != m_pReportCenter)
		{
			m_pReportCenter->reportSendTo("mainui", PACKET_FUN_QUERY_SYS_STATUS, (OS_LPARAM)&status, 0);
		}
		packet->status = PACKET_STATUS_OK;
		char bodybuf[256] = { 0 };
		sprintf(bodybuf, "{\"sysStatus\":\"%d\"}", status);
		packet->body = bodybuf;
    }
	else if (packet->fun == PACKET_FUN_QUERY_PRODUCT_SN)
	{
		packet->body = "";
		packet->status = PACKET_STATUS_OK;
		unsigned int iProSN = 0;
		if (NULL != m_pReportCenter)
		{
			m_pReportCenter->reportSendTo("mainui", PACKET_FUN_QUERY_PRODUCT_SN, (OS_LPARAM)&iProSN, 0);
		}
		packet->status = PACKET_STATUS_OK;
		char bodybuf[256] = { 0 };
		sprintf(bodybuf, "{\"productSn\":\"%d\"}", iProSN);
		packet->body = bodybuf;
	}
    else if (packet->fun == PACKET_FUN_SEND_ALARM)
    {
        packet->body = "";
        packet->status = PACKET_STATUS_OK;
        unsigned int iProSN = 0;
        if (NULL != m_pReportCenter)
        {
            m_pReportCenter->reportSendTo("mainui", PACKET_FUN_SEND_ALARM, 0, 0);
        }
    }
...
int CSocketproxy::sendSysPause()
{
	return sendRequest(PACKET_FUN_SYS_PAUSE);
}
...
int CSocketproxy::sendQuerySysStatus(int & iStatus)
{
	stPacket packet;
	packet.fun = PACKET_FUN_QUERY_SYS_STATUS;
	int iRet = sendRequestPacket(&packet);
	if (iRet != ERR_OK)
	{
		return iRet;
	...
	iRet = waitAckPacket(packet.sn, &body);
	if (iRet == ERR_OK)
	{
		if (body.find("sysStatus") != body.end())
		{
			iStatus = SString::strtoInt(body["sysStatus"]);
		}
	}
	return iRet;
}

int CSocketproxy::sendQueryProductSN(unsigned int& isn)
{
	stPacket packet;
	packet.fun = PACKET_FUN_QUERY_PRODUCT_SN;
	int iRet = sendRequestPacket(&packet);
	if (iRet != ERR_OK)
	{
		return iRet;
	...
	if (iRet == ERR_OK)
	{
		if (body.find("productSn") != body.end())
		{
			isn = SString::strtoInt(body["productSn"]);
		}
	}
	return iRet;
}
int CSocketproxy::connectSocket()
{
	if (m_CSocketClient != NULL && !m_CSocketClient->isConnected())
	{
		return m_CSocketClient->connectServer();
	}
	return 0;
}
