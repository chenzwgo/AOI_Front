...
private:
	...
	static void CallbackGrapFunctionSecond(void *, stImageInfo*);// 回调函数
	static void CallbackGrapFunctionThree(void *, stImageInfo*);// 回调函数

	void ThreadImageForCamer(void* param);
private:
	CHkImageLogic*	m_ImageProInstance[SEQUE_NUMBER];

	JThreadTP<CHkImageLogicManager, void*, void>*	m_pThreadImageForCamer;
	...
	ICamera*		m_pCameraInstance[CAMERA_NUMBER];
	VS_CAMERA_INFO	m_pCameraInfo;
	stImageInfo		m_ImageInfo[SEQUE_NUMBER];
	//
	stImageInfo		m_OneImageInfo[CAMERA_NUMBER][SEQUE_NUMBER];
	//
    string m_strScene;
    int m_iIndex;
	...
	int m_iOverSize;
	int m_iCamerCnt;//相机个数
	int m_iChanelIndex[SEQUE_NUMBER];
	bool m_bChanelSend[CAMERA_NUMBER][SEQUE_NUMBER];
};
