...
FilterSet::FilterSet(vector<defectlevel>* VecDefect, int iRegion,QWidget *parent) :
    QWidget(parent),
	m_iRegion(iRegion),
	m_VecCurrData(NULL),
	m_pScriptLevel(NULL),
...
bool FilterSet::execSingle(QString strExpr, itemValue& value)
{
	if (strExpr.length() == 0)
	{
...
void FilterSet::UiCancel()
{
	InitUi();
}

vector<defectlevel>* FilterSet::getUiVale()
{
...
void FilterSet::slotActExec()
{
	...
	typeitem m_ptypeitem(&itemValuetmp);
...
void FilterSet::slotActLevelExec()
{
	/* 2021lu
	bool bError = false;
	int iTopCnt = m_ptreeWidget->topLevelItemCount();
	...
	if (m_pIDefectQualification != NULL)
	{
		...
		{
			if (m_strDefectName.compare(szdefectTypeEn[i]) == 0)
			{
...
void FilterSet::popMenu(const QPoint& pot)
{
...
void FilterSet::slotitemDoubleClicked(QTreeWidgetItem* item, int column)
{
	m_pCurrItem = item;
}

void FilterSet::slotitemSelectionChanged()
{
	if (m_pCurrItem == NULL)
	{
		return;
