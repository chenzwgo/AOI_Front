...
#define  SOCKET_FUN_FILE_INI_PATH "./plugins/mainui/FunConfig.ini"
...
{
    CSocketClientPC1* pISMCameraClient = m_pSocketServer->getSocketClient(m_strCameraName.toStdString());
...
{
    CSocketClientPC1* pSocketClient = m_pSocketServer->getSocketClient(m_strCameraName.toStdString());
    ...
    int iErr = ERR_OK;
    iErr = pSocketClient->sendRequest(stContext.strFunCode.toStdString(),stContext.strBody.toStdString());
...
void CommunicationTestForm::readFunCfgInI()
{
    QString strPath = SOCKET_FUN_FILE_INI_PATH;
    ...
    {
        QString strKey = GroupList.at(i);
        setting.beginGroup(strKey);      
        STSendContext* pContext = new STSendContext;
        pContext->strFunCode =  setting.value("Fun").toString();
        pContext->strBody = setting.value("Body").toString();
        pContext->strDes = setting.value("des").toString();
        m_SendContextMap.insert(pContext->strFunCode,pContext);
        setting.endGroup();
    }
