...
int runSV3(QString threadName)
{
	QString appPath = QCoreApplication::applicationDirPath();
	dv::BJInputParam bjInputParam;
	const char* config_file = "D:/config/config.json";
	dv::DefectResultInfo defectResult[5] = { NULL };
	InitBJParam(&bjInputParam,config_file);
	//≥ı ºªØ log
	initLogger(LOG_DEBUG | LOG_ERROR | LOG_WARNING);
	std::vector<QString> img_namevec;
	img_namevec.push_back("image/procuct(111)-SEQUENCE_A_10-18-55_rawImge.bmp");
	img_namevec.push_back("image/procuct(111)-SEQUENCE_B_10-18-54_rawImge.bmp");
	img_namevec.push_back("image/procuct(111)-SEQUENCE_C_10-18-54_rawImge.bmp");
	img_namevec.push_back("image/procuct(111)-SEQUENCE_D_10-18-55_rawImge.bmp");
	bjInputParam.image = new dv::Image[4];
	for (int i = 0; i<4; i++)
	{
	...
	for (size_t i = 0; i < 2; i++)
	{
		...
		{
			setImageName(std::to_string(count).c_str(), 4, k);
			count++;
		}

		int start_timer = 0;
		int ret = classifyWhiteDetect(&bjInputParam, defectResult);
